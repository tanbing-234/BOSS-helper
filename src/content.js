...
function extractCurrentJob() {
  ...
  const description = cleanJobDescription(textOf(root, SELECTORS.description) || inferDescription(root));
  const company = extractCompanyName(document);
...
function extractCardData(card) {
  ...
  let company = cleanCompanyName(textOf(card, [".company-name", ".company-info .name", ".company-text .name", "[class*='company-name']"]));
  if (!company) {
    company = [...lines].reverse().map(cleanCompanyName).find((line) => isLikelyCompanyLine(line, title, salary)) || "";
  }
...
function isLikelyCompanyLine(line, title, salary) {
  if (!line || line === title || (salary && line.includes(salary)) || line.length > 40) return false;
  if (/^(经验不限|学历不限|\d+[-–]\d+年|本科|大专|硕士|博士|应届生)/.test(line)) return false;
  if (isLocationOnly(line) || /^(北京|上海|天津|重庆|深圳|广州|杭州|成都|武汉|西安|南京|苏州|长沙|郑州|青岛|厦门|合肥|昆明|宁波|东莞)[··]/.test(line)) return false;
  return true;
}

function extractCompanyName(scope) {
  const candidates = SELECTORS.company.flatMap((selector) => [...scope.querySelectorAll(selector)])
    .map((element) => cleanCompanyName(element.innerText || element.textContent || ""))
    .filter((value) => isLikelyCompanyLine(value, "", ""));
  return candidates.sort((a, b) => companyNameScore(b) - companyNameScore(a) || a.length - b.length)[0] || "";
}

function cleanCompanyName(value) {
  return decodeBossText(value).split(/\n+/).map((line) => line.trim()).filter(Boolean)
    .find((line) => !isLocationOnly(line) && line.length <= 60) || "";
}

function companyNameScore(value) {
  return (/公司|集团|科技|事务所|研究院|中心|银行|学校|医院|商贸|咨询|有限/.test(value) ? 20 : 0) + Math.min(value.length, 20);
}

function isLocationOnly(value) {
  return /^(?:北京|上海|天津|重庆|深圳|广州|杭州|成都|武汉|西安|南京|苏州|长沙|郑州|青岛|厦门|合肥|昆明|宁波|东莞)(?:市|市区|地区)?(?:[·\s-].*)?$/.test(String(value || "").trim());
}

function cleanJobDescription(value) {
  const lines = decodeBossText(value).replace(/\r/g, "").split(/\n+/).map((line) => line.trim()).filter(Boolean);
  ...
  const cleaned = (firstContent > 0 ? lines.slice(firstContent) : lines).filter((line) => !noise.test(line));
  return cleaned.join("\n").trim();
}
...
function extractZhilianJobDetail() {
  ...
  const description = cleanJobDescription(inferZhilianDescription(root));
  const company = cleanCompanyName(textOf(document, ["[class*='company-name']", "[class*='company-title']"]))
