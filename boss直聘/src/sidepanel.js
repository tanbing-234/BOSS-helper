...
async function startZhilianAutoApply(scheduled = false) {
  ...
  try {
    ...
    run = {
      ...
      startedAt: new Date().toISOString(), mode: scheduled ? "scheduled_zhilian" : "zhilian", appliedJobs: []
    };
    autoRun = run;
    autoStarting = false;
...
async function processZhilianJob(sourceTab, queuedJob, settings, run, keyword) {
  ...
  try {
    ...
    const job = {
      ...detail.job,
      company: queuedJob.company || detail.job.company,
    ...
    run.applied += 1;
    run.appliedJobs.push({ company: job.company, jobTitle: job.title, salary: job.salary || queuedJob.salary || "未识别", workSummary: summarizeWorkContent(job.description), score: result.score, url: job.url });
...
async function processAutoJob(tab, queuedJob, settings, keyword, run) {
  ...
  try {
      ...
      if (!shouldApply) {
        ...
        const archiveJob = {
          ...job,
          company: queuedJob.company || job.company,
