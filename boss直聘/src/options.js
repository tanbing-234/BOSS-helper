...
async function load() {
  ...
  if (repaired.changed) {
    await chrome.storage.local.set({ applicationWhitelist: repaired.whitelist, applicationArchives: repaired.archives });
  }
    ...
    for (const job of report.jobs || []) {
      ...
      if (!hasArchive(job.company, job.jobTitle)) {
        resultArchives.push({
          ...
          description: job.workSummary || "", score: job.score ?? null, matchedStrengths: [], greeting: "",
...
async function exportWhitelistCsv() {
  ...
  const rows = entries.map((entry) => {
    ...
    return [
      ...
      archive?.score ?? "", archive?.appliedAt || entry.addedAt || "", archive?.description || "",
